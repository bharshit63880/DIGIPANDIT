    ...
    <header ref={headerRef} className={`cinematic-navbar dp-navbar inset-x-0 top-0 z-50 text-white ${isCinematicHome ? "fixed" : "sticky"} ${isAstrology ? "dp-navbar--reference" : ""} ${isScrolled ? "is-scrolled" : ""}`}>
      <div className="container-shell">
        <div className="grid min-h-[76px] grid-cols-[auto_1fr_auto] items-center gap-4">
          ...
          <div className="dp-navbar-actions flex min-w-0 items-center justify-self-end gap-3">
            ...
            {user ? <><Link to={user.role === "ADMIN" ? "/admin" : user.role === "PANDIT" ? "/pandit-dashboard" : "/dashboard"}><Button variant="secondary" className="whitespace-nowrap">{user.role === "USER" ? "प्रोफ़ाइल" : "डैशबोर्ड"}</Button></Link><Button onClick={handleLogout} className="hidden whitespace-nowrap sm:inline-flex">लॉग आउट</Button></> : <><Link to="/login" className="hidden sm:block"><Button variant="secondary" className="dp-auth-login">लॉग इन</Button></Link><Link to="/register"><Button className="dp-auth-register">खाता बनाएं</Button></Link></>}
