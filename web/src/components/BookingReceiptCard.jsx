...
export function BookingReceiptCard({ booking, title = "भुगतान रसीद", compact = false }) {
    ...
    <div className={`rounded-[28px] border border-brand-sand/70 bg-white ${compact ? "p-5" : "p-6"} shadow-soft`}>
      ...
      <div className={`mt-6 grid gap-4 ${compact ? "md:grid-cols-2" : "md:grid-cols-3"}`}>
        <div className="rounded-[22px] bg-brand-cream/70 p-4">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-clay">निर्धारित समय</p>
          <p className="mt-2 text-sm font-semibold text-brand-ink">{formatDateTime(booking.scheduleAt)}</p>
        </div>
        <div className="rounded-[22px] bg-brand-cream/70 p-4">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-clay">भुगतान का समय</p>
          <p className="mt-2 text-sm font-semibold text-brand-ink">{formatDateTime(booking.payment?.paidAt)}</p>
        </div>
        {!compact ? (
          <div className="rounded-[22px] bg-brand-cream/70 p-4">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-clay">भुगतान संदर्भ</p>
            <p className="mt-2 break-all text-sm font-semibold text-brand-ink">{booking.payment?.gatewayPaymentId || "लंबित"}</p>
          </div>
        ) : null}
      </div>
      ...
      {!compact && booking.address ? (
        <div className="mt-6 rounded-[24px] border border-brand-sand/70 p-4">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-clay">सेवा का पता</p>
