...
export default function AdminDashboardPage() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return query
      ? users.filter((user) => [user.name, user.email, user.role, user.city].some((value) => value?.toLowerCase().includes(query)))
      : users;
  }, [userSearch, users]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return query
      ? products.filter((product) => [product.name, product.category, product.slug].some((value) => value?.toLowerCase().includes(query)))
      : products;
  ...
  const toggleHawanPublish = async (hawanId) => {
    await api.patch(`/admin/hawans/${hawanId}/publish`);
    setNotice("Hawan publication status updated.");
    loadData();
  };

  const duplicateHawan = async (hawanId) => {
    await api.post(`/admin/hawans/${hawanId}/duplicate`);
    setNotice("Hawan guide duplicated as an unpublished copy.");
    loadData();
  };

  const archiveHawan = async (hawanId) => {
    if (!window.confirm("Archive this Hawan guide? It will disappear from the public library.")) return;
    await api.delete(`/admin/hawans/${hawanId}`);
    setNotice("Hawan guide archived.");
    loadData();
  };
  ...
  if (loadError || !dashboard) {
    return (
      <div className="rounded-[36px] bg-white p-8 shadow-soft">
        <h1 className="text-3xl font-bold">Admin panel could not load</h1>
        <p className="mt-3 text-brand-ink/65">{loadError || "Please sign in with an active admin account."}</p>
      ...
      {activeTab === "hawans" ? (
        <div className="rounded-[36px] bg-white p-8 shadow-soft">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="eyebrow">Structured ritual content</p>
              <h2 className="mt-2 text-3xl font-bold text-brand-ink">Hawan guide management</h2>
              <p className="mt-2 text-sm text-brand-ink/65">Publish, unpublish, duplicate, preview, or safely archive guides. Complete structured creation and updates are protected by the admin API.</p>
            </div>
            <a href="/admin/hawans/new/edit"><Button>Create Hawan guide</Button></a>
          </div>
          <div className="mt-7 grid gap-4">
            {hawans.map((hawan) => (
              <div key={hawan._id} className="rounded-[24px] border border-brand-sand p-5 md:flex md:items-center md:justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-clay">{hawan.category} · {hawan.isArchived ? "Archived" : hawan.isPublished ? "Published" : "Draft"}</p>
                  <h3 className="mt-2 text-2xl font-bold">{hawan.title}</h3>
                  <p className="mt-2 text-sm text-brand-ink/65">{hawan.steps?.length || 0} steps · {hawan.materials?.length || 0} materials · {hawan.mantras?.length || 0} mantras</p>
                </div>
                <div className="mt-4 flex flex-wrap gap-2 md:mt-0">
                  {!hawan.isArchived ? <a href={`/hawan-guide/${hawan.slug}`} target="_blank" rel="noreferrer"><Button variant="secondary">Preview</Button></a> : null}
                  <a href={`/admin/hawans/${hawan._id}/edit`}><Button variant="secondary">Edit</Button></a>
                  <Button variant="secondary" onClick={() => duplicateHawan(hawan._id)}>Duplicate</Button>
                  <Button onClick={() => toggleHawanPublish(hawan._id)}>{hawan.isPublished ? "Unpublish" : "Publish"}</Button>
                  {!hawan.isArchived ? <Button variant="ghost" onClick={() => archiveHawan(hawan._id)}>Archive</Button> : null}
                </div>
              </div>
            ))}
            {!hawans.length ? <p className="text-sm text-brand-ink/65">No Hawan guides found. Run the structured Hawan seeder once, then manage them here.</p> : null}
          </div>
        </div>
      ) : null}
      ...
      {activeTab === "users" ? (
        <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className="rounded-[36px] bg-white p-8 shadow-soft">
            ...
            <div className="mt-5">
              <Input label="Search users" placeholder="Name, email, role or city" value={userSearch} onChange={(event) => setUserSearch(event.target.value)} />
            </div>
            <div className="mt-6 space-y-4">
      ...
      {activeTab === "products" ? (
        <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className="rounded-[36px] bg-white p-8 shadow-soft">
            ...
            <div className="mt-5">
              <Input label="Search products" placeholder="Name, category or slug" value={productSearch} onChange={(event) => setProductSearch(event.target.value)} />
            </div>
            <div className="mt-6 space-y-4">
