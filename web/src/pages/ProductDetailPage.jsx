...
export default function ProductDetailPage() {
    ...
    const loadProduct = async () => {
      try {
        ...
        if (!allowedStoreCategories.includes(nextProduct.category)) {
          setError("यह सामग्री अभी पूजा स्टोर में उपलब्ध नहीं है।");
          setProduct(null);
          return;
        }
        ...
        setError("अभी सामग्री का विवरण लोड नहीं हो सका। कृपया थोड़ी देर बाद फिर कोशिश करें।");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="container-shell py-16 text-sm text-brand-ink/70">सामग्री का विवरण लोड हो रहा है...</div>;
  }
    ...
    return (
      <div className="container-shell py-16">
        <div className="rounded-[28px] bg-white p-8 shadow-soft">
          <p className="text-sm text-brand-ink/70">{error || "यह सामग्री नहीं मिली।"}</p>
          <Link to="/store" className="mt-5 inline-flex">
            <Button variant="secondary">पूजा स्टोर पर वापस जाएँ</Button>
          </Link>
        </div>
      </div>
    );
    ...
    <div className="dp-theme dp-store-theme container-shell py-12">
        ...
        <div className="rounded-[32px] bg-white p-8 shadow-soft">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            <div className="rounded-[22px] bg-brand-sand/35 p-4">
              ...
              <p className="mt-3 text-sm font-semibold text-brand-ink">व्यावहारिक पूजा उपयोग के लिए तैयार</p>
            </div>
            <div className="rounded-[22px] bg-brand-sand/35 p-4">
              <Sparkles className="h-5 w-5 text-brand-maroon" />
              <p className="mt-3 text-sm font-semibold text-brand-ink">घर और मंदिर, दोनों के लिए उपयुक्त</p>
            </div>
            <div className="rounded-[22px] bg-brand-sand/35 p-4">
              <ShieldCheck className="h-5 w-5 text-brand-maroon" />
              <p className="mt-3 text-sm font-semibold text-brand-ink">सुरक्षित पैकिंग और स्पष्ट उपयोग निर्देश</p>
            </div>
          </div>

          <div className="mt-8 flex flex-wrap gap-4">
            <Button onClick={() => dispatch(addToCart(product))}>कार्ट में जोड़ें</Button>
            <Link to="/cart">
              <Button variant="secondary">कार्ट देखें</Button>
            </Link>
          </div>

          {savings > 0 ? (
            <p className="mt-4 text-sm font-medium text-brand-forest">इस सामग्री पर आपकी ₹{savings} तक की बचत हो रही है।</p>
          ) : null}
        </div>
      </div>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_1fr]">
        <section className="rounded-[30px] bg-white p-7 shadow-soft">
          <h2 className="text-2xl font-bold text-brand-ink">पैकेट में क्या शामिल है</h2>
        ...
        <section className="rounded-[30px] bg-white p-7 shadow-soft">
          <h2 className="text-2xl font-bold text-brand-ink">यह सामग्री किसके लिए उपयोगी है</h2>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="rounded-[30px] bg-white p-7 shadow-soft">
          <h2 className="text-2xl font-bold text-brand-ink">उपयोग विधि</h2>
        ...
        <aside className="rounded-[30px] bg-white p-7 shadow-soft">
          <h2 className="text-2xl font-bold text-brand-ink">महत्वपूर्ण सूचना</h2>
          <p className="mt-4 text-sm leading-8 text-brand-ink/75">{content.note}</p>

          {product.tags?.length ? (
            <>
              <h3 className="mt-7 text-lg font-bold text-brand-ink">लोकप्रिय टैग</h3>
