...
const upsertUser = async (payload) => {
  ...
  if (!user) {
    ...
    // Preserve an existing local user's password when adding showcase data.
    // New demo accounts still receive the standard demo password above.
  ...
  const bookingPayloads = [
    {
      bookingType: "PUJA",
      status: "COMPLETED",
      user: userMap["bharshit63880@gmail.com"]._id,
      ...
      meetingMode: "OFFLINE",
      address: userMap["bharshit63880@gmail.com"].addresses[0],
      payment: {
        status: "PAID",
  ...
  const orders = [
    {
      ...
      shippingAddress: userMap["bharshit63880@gmail.com"].addresses[0],
...
const createChats = async ({ bookings, userMap, panditMap }) => {
  const grihaPraveshBooking = bookings.find((item) => item.serviceName === "Griha Pravesh Puja");
  ...
  const roomThree = await ChatRoom.create({
    booking: grihaPraveshBooking._id,
    participants: [userMap["bharshit63880@gmail.com"]._id, panditMap["raghav.pandit@digipandit.demo"]._id],
    lastMessage: "पूजा से एक दिन पहले सामग्री सूची और समय की अंतिम पुष्टि कर लेंगे।",
    lastMessageAt: new Date("2026-07-26T10:30:00.000Z"),
  });
  ...
  await Message.insertMany([
    ...
    {
      room: roomThree._id,
      sender: userMap["bharshit63880@gmail.com"]._id,
      content: "नमस्ते पंडित जी, गृह प्रवेश पूजा के लिए समय और आवश्यक सामग्री की पुष्टि कर दीजिए।",
      readBy: [userMap["bharshit63880@gmail.com"]._id, panditMap["raghav.pandit@digipandit.demo"]._id],
      createdAt: new Date("2026-07-26T10:05:00.000Z"),
      updatedAt: new Date("2026-07-26T10:05:00.000Z"),
    },
    {
      room: roomThree._id,
      sender: panditMap["raghav.pandit@digipandit.demo"]._id,
      content: "नमस्ते। पूजा से एक दिन पहले सामग्री सूची और समय की अंतिम पुष्टि कर लेंगे।",
      readBy: [panditMap["raghav.pandit@digipandit.demo"]._id],
      createdAt: new Date("2026-07-26T10:30:00.000Z"),
      updatedAt: new Date("2026-07-26T10:30:00.000Z"),
    },
  ]);
};
