...
const bookPandit = asyncHandler(async (req, res) => {
  ...
  // All monetary values are derived from the approved service; never trust browser-supplied prices.
  const resolvedTravelCharge = meetingMode === "ONLINE" ? 0 : service.travelCharge || 0;
  const resolvedSamagriCost = service.samagriCost || 0;
  const resolvedExtraDakshina = service.extraDakshina || 0;
  const resolvedVideoDakshinaFee = meetingMode === "ONLINE" ? service.videoDakshinaFee || 0 : 0;
