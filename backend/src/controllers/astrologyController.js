...
const { searchIndianCities } = require("../services/citySearchService");
const {
  getDailyGuidance,
  buildAdvancedReport,
  analyzeDoshas,
  numerology,
  matchKundalis,
} = require("../services/astrologyInsightsService");
...
const listAstrologers = asyncHandler(async (req, res) => {
  ...
  if (specialization) {
    filter.specialization = { $regex: escapeRegex(specialization.trim().slice(0, 100)), $options: "i" };
  }

  if (language) {
    filter.languages = { $in: [new RegExp(escapeRegex(language.trim().slice(0, 100)), "i")] };
  }
...
const endSession = asyncHandler(async (req, res) => {
  ...
  const finalStatus = session.expectedAutoEndAt && now > session.expectedAutoEndAt ? "AUTO_ENDED" : "ENDED";
  const endedSession = await ConsultationSession.findOneAndUpdate(
    { _id: session._id, status: "ACTIVE" },
    {
      $set: {
        status: finalStatus,
        endedBy: finalStatus === "AUTO_ENDED" ? "SYSTEM" : req.user.role === "ADMIN" ? "ADMIN" : "USER",
  ...
  if (!endedSession) {
    const currentSession = await ConsultationSession.findById(session._id);
    return res.json({ success: true, message: "Consultation session already ended", data: currentSession });
  }

  const user = await User.findOneAndUpdate(
    { _id: session.user, "wallet.balance": { $gte: calculatedAmount } },
    { $inc: { "wallet.balance": -calculatedAmount } },
    { new: true }
  );
  const chargeAmount = user ? calculatedAmount : 0;
  ...
  endedSession.billedAmount = chargeAmount;
  endedSession.walletBalanceAfter = user?.wallet?.balance ?? session.walletBalanceBefore;
  ...
  res.json({
    ...
    data: {
      ...
      wallet: user?.wallet || { balance: session.walletBalanceBefore, currency: "INR" },
    },
  });
});
...
const createKundali = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    data: {
      kundali,
      interpretation,
      report: buildAdvancedReport(kundali, interpretation),
      doshas: analyzeDoshas(kundali),
    },
  });
});

const getDailyAstrology = asyncHandler(async (req, res) => {
  res.json({ success: true, data: getDailyGuidance(req.query.date) });
});

const createMatch = asyncHandler(async (req, res) => {
  res.json({ success: true, data: matchKundalis(req.validated.body.bride, req.validated.body.groom) });
});

const createNumerology = asyncHandler(async (req, res) => {
  res.json({ success: true, data: numerology(req.validated.body) });
});

const searchCities = asyncHandler(async (req, res) => {
  res.json({ success: true, data: searchIndianCities(req.query.q) });
});
