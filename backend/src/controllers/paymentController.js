...
function amountsMatch(expected, actual) {
  return roundCurrency(Number(expected)) === roundCurrency(Number(actual));
}
...
const payuCallback = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!entity.payment?.gatewayOrderId || entity.payment.gatewayOrderId !== payload.txnid) {
      throw new ApiError(StatusCodes.BAD_REQUEST, "Payment transaction does not match the payable item");
    }

    if (!amountsMatch(getEntityAmount(entityType, entity), amount)) {
      throw new ApiError(StatusCodes.BAD_REQUEST, "Payment amount does not match the payable item");
    }
