...
const updateBookingStatus = asyncHandler(async (req, res) => {
  ...
  const updatedBooking = await Booking.findOneAndUpdate(
    { _id: booking._id, status: { $in: allowedPreviousStatuses[status] } },
    { $set: { status } },
    { new: true, runValidators: true }
  );

  if (!updatedBooking) {
    throw new ApiError(StatusCodes.CONFLICT, "This booking has already moved to a different status");
  }

  if (status === "COMPLETED" && updatedBooking.payment.status === "PAID") {
    await PanditProfile.findOneAndUpdate(
      { user: updatedBooking.pandit },
      {
        $inc: {
          totalBookings: 1,
          totalEarnings: updatedBooking.payment.amount,
        },
      }
    );
  }
